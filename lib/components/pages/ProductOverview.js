...
class ProductOverview extends Component {
  ...
  getImage(product) {
    return product.mediaLinks.length? new db.File(product.mediaLinks[0]).url : 'http://www.myiconfinder.com/uploads/iconsets/256-256-323c59a9f2afd00a83d41dcf738c5978.png'
  }
  ...
  getStars(rating) {
    ...
    for (var i = 0; i < whole; i++) {
      result.push(<span><i className="fa fa-star"></i></span>)
    }
    if (rating - whole > 0){
      result.push(<span><i className="fa fa-star-half-o"></i></span>)
      empty--;
    }
    for (var i = 0; i < empty; i++) {
      result.push(<span><i className="fa fa-star-o"></i></span>)
    }
    return result;
  }
